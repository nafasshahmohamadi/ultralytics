    ...
    def plot_training_labels(self):
        ...
        for lb in self.train_loader.dataset.labels:
            ...
            if box.shape[1] == 8:  # It's a polygon (x1,y1,x2,y2,x3,y3,x4,y4)
                # Convert polygon to an axis-aligned bounding box (xyxy)
                x = box[:, 0::2]
                y = box[:, 1::2]
                box_xyxy = np.concatenate([x.min(1, keepdims=True), y.min(1, keepdims=True),
                                           x.max(1, keepdims=True), y.max(1, keepdims=True)], axis=1)
                # Convert xyxy to xywh for plotting
                boxes.append(xyxy2xywh(box_xyxy))
